...
module.exports.run = async ({ api, event, Currencies }) => {
  ...
  if (event.senderID !== adminUID) {
    return api.sendMessage("Yeh command sirf bot admin ke liye hai!", event.threadID, event.messageID);
  }
  ...
  if (money >= 1000) {
    axios.get('http://api.vangbanlanhat.tk/image?type=sexy').then(res => {
      ...
      request(res.data.data)
        .pipe(fs.createWriteStream(__dirname + '/cache/trai.jpg'))
        .on("close", callback)
        .then(Currencies.setData(event.senderID, options = { money: money - 1000 }));
    });
  } else {
